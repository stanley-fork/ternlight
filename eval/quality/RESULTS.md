...
**Story in one line.** From the fp32 teacher down to the smallest shipped
variant: a **31×** reduction in bin size costs **0.29** Spearman.
...
**Read.** Int8 embedding holds 95% of fp32's pair-ranking signal at 4.5× the
compression. Going further to ternary embedding drops to 80% — the cliff is at
the embedding lookup, not the linear weights (which are ternary in both).
...
- **Where each number comes from.**
  ...
  - ternlight variants: WASM eval via `eval/quality/spearman.js` against the built `engine/pkg/`. Each variant required rebuilding the engine with the matching feature flag (`emb_int8` / `emb_ternary`) and swapping the `.bin`.
